...
public class TrailRendererWith2DCollider : MonoBehaviour {
	...
	[SerializeField]
	private Material trailMaterial;					//the material of the trail.  Changing this during runtime will have no effect.
	[SerializeField]
	private float lifeTime = 1.0f;                   //the amount of time in seconds that the trail lasts
	[SerializeField]
	...
	[SerializeField]
	private float widthEnd = 1.0f;                   //the ending width of the trail
	[SerializeField]
	private float vertexDistanceMin = 0.10f;         //the minimum distance between the center positions
	[SerializeField]
	private Vector3 renderDirection = new Vector3(0, 0, -1); //the direction that the mesh of the trail will be rendered towards
	[SerializeField]
	private bool colliderIsTrigger = true;           //determines if the collider is a trigger.  Changing this during runtime will have no effect.
	[SerializeField]
	private bool colliderEnabled = true;             //determines if the collider is enabled.  Changing this during runtime will have no effect.
	[SerializeField]
	private bool pausing = false;                     //determines if the trail is pausing, i.e. neither creating nor destroying vertices
	
	private Transform trans;                        //transform of the object this script is attached to                    
	private Mesh mesh;
    private DestroyMe destroyMeScript;                   
	...
	private void Awake() {
		...
		GameObject trail = new GameObject("Trail", new[] { typeof(MeshRenderer), typeof(MeshFilter), typeof(PolygonCollider2D), typeof(DestroyMe)} );
        trail.tag = "Trail";
        destroyMeScript = trail.GetComponent<DestroyMe>();
        destroyMeScript.timer = 5;
